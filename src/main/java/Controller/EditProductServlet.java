...
public class EditProductServlet extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response) throws IOException, ServletException {
        ...
        int productId = Integer.parseInt(request.getParameter("productId"));
        Product product = productDB.getProductById(productId);
        if (FileUpload.getFileName(request.getPart("img")).isEmpty() ||
                FileUpload.getFileName(request.getPart("img1")).isEmpty() ||
                FileUpload.getFileName(request.getPart("img2")).isEmpty() ||
                FileUpload.getFileName(request.getPart("img3")).isEmpty()) {
            ...
            String img = FileUpload.uploadFile(request, getServletContext(), request.getPart("img"), FileUpload.getFileName(request.getPart("img")));
            String img1 = FileUpload.uploadFile(request, getServletContext(), request.getPart("img1"), FileUpload.getFileName(request.getPart("img1")));
            String img2 = FileUpload.uploadFile(request, getServletContext(), request.getPart("img2"), FileUpload.getFileName(request.getPart("img2")));
            String img3 = FileUpload.uploadFile(request, getServletContext(), request.getPart("img3"), FileUpload.getFileName(request.getPart("img3")));
            product.setImg(img);
